"""
VaultGate — Manim Slides Presentation
Render: manim-slides render --GL vaultgategl.py VaultGatePitch
Present: manim-slides VaultGatePitch
Export HTML: manim-slides convert VaultGatePitch --to=html
"""
...
# ═════════════════════════════════════════════════════════════════════
class VaultGatePitch(Slide):
    def construct(self):
        self.camera.background_color = BG
        self.scene_1_horror()
        self.next_slide()  # → Slide 2: Reframe
        self.scene_2_reframe()
        self.next_slide()  # → Slide 3: VaultGate
        self.scene_3_vaultgate()
        self.next_slide()  # → Slide 4: Demo
        self.scene_4_demo()
        self.next_slide()  # → Slide 5: Wins
        self.scene_5_wins()
        self.next_slide()  # → Slide 6: Future
        self.scene_6_future()
        self.next_slide()  # → Slide 7: CTA
        self.scene_7_cta()
        self.next_slide()  # → End
    ...
    def node_clean(self, label, color=ACCENT, w=2.2, h=1.0):
        """Simple rounded rectangle node with label."""
        box = RoundedRectangle(corner_radius=0.1, width=w, height=h, fill_color=DARK_BOX, stroke_color=color, stroke_width=2)
        lbl = Text(label, font_size=16, color=TEXT, weight=BOLD).move_to(box)
    ...
    def scene_3_vaultgate(self):
        ...
        # Title
        title = heading("VaultGate Architecture", size=48, color=ACCENT)
        self.play(Write(title), run_time=0.8)
        self.wait(0.5)
        
        # Simple flow: AI Agent → VaultGate → Gate → Phone
        ai = self.node_clean("AI Agent", MUTED, 2.0, 0.8).shift(LEFT * 5)
        vg = self.node_clean("VaultGate", ACCENT, 2.4, 1.0).shift(LEFT * 1.5)
        gate = hex_gate("GATE", WARN, 2.0).shift(RIGHT * 2)
        phone = self.node_clean("Phone", SAFE, 1.6, 0.8).shift(RIGHT * 5)
        
        self.play(Write(ai), run_time=0.4)
        self.play(Write(vg), run_time=0.4)
        self.play(Write(gate), run_time=0.4)
        self.play(Write(phone), run_time=0.4)
        
        # Arrows
        arr1 = Arrow(ai.get_right(), vg.get_left(), buff=0.2, color=MUTED, stroke_width=5)
        arr2 = Arrow(vg.get_right(), gate.get_left(), buff=0.2, color=WARN, stroke_width=5)
        arr3 = Arrow(gate.get_right(), phone.get_left(), buff=0.2, color=SAFE, stroke_width=5)
        self.play(GrowArrow(arr1), GrowArrow(arr2), GrowArrow(arr3), run_time=0.6)
        
        # Labels
        lbl1 = body_text("requests", size=14, color=MUTED).next_to(arr1, DOWN, buff=0.15)
        lbl2 = body_text("validates", size=14, color=ACCENT).next_to(arr2, DOWN, buff=0.15)
        lbl3 = body_text("CIBA push", size=14, color=SAFE).next_to(arr3, DOWN, buff=0.15)
        self.play(Write(lbl1), Write(lbl2), Write(lbl3), run_time=0.5)
